...
# Wykies Automation — Starter (v6)

Generated: 2026-01-07T19:08:42.002090

## What’s inside
- Public site (`index.html`, assets)
- Admin (`/admin`) with passphrase login and role‑based UI
- `config.json` for your Apps Script Web App URL and Sheet ID

## Quick start
1. **Create Apps Script** (see `apps_script_backend.gs` below) and deploy as Web App. Copy the Web App URL into `config.json > scriptUrl`.
2. **Set Admin Passphrase** in Apps Script (`ADMIN_PASSPHRASE_HASH`), using SHA‑256 of your secret.
3. **Host** this folder on GitHub Pages or any static host.

## Fix for Missing Buttons
- The **Add Product** and **Bump Version** buttons are marked `data-role="admin-only"`. They are shown only when the backend returns `role: 'admin'` on `/login` or `/session`.
- If you signed in but still don’t see the button, check:
  - `localStorage.wykies_admin_session` exists and has a `token`.
  - The `/session` endpoint returns `{ ok: true, role: 'admin' }`.
  - No CSS rule hides buttons: in `admin/styles.css`, `.hidden { display:none }` and we toggle only on role.

## Google Apps Script Backend (paste into Code.gs)
```js
// ===== CONFIG =====
...
### How to set the admin passphrase hash
Open Apps Script **Project Settings → Script properties** and add:
- `SHEET_ID` = your Google Sheet ID
- `ADMIN_PASS_HASH` = SHA‑256 hex of your secret (use any offline tool)

### Deploy as Web App
- **Deploy → New Deployment → Web app**
- Execute as: **Me**
- Who has access: **Anyone** (security is enforced via token)
- Copy the Web App URL and paste into `config.json > scriptUrl`

```
"scriptUrl": "https://script.google.com/macros/s/AKfycbw.../exec"
```

Then open `/admin/` and sign in with your passphrase.
